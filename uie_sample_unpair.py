...
from uie_codes.UIEC2Net import UIEC2Net
from uie_codes.Classifier import Discriminator
from uie_codes.UIEB_unpair import UIEBDataset
...
def main():
    ...
    UIEDataset = UIEBDataset("/home/iscas/ddz/UIE/data/UIEB/challenging-60",  # "/home/iscas/ddz/UIE/data/UIEB/challenging-60"
                             "/home/iscas/ddz/UIE/data/UIEB/All_Results/Fusion/C60",
                             "/home/iscas/ddz/UIE/data/UIEB/challenging-60-depth",
                             image_size=args.image_size)
    # UIEDataset = UIEBDataset("/home/iscas/ddz/UIE/data/UIEB/challenging-60",  # 
    #                          "/home/iscas/ddz/UIE/data/UIEB/All_Results/UIEC2Net/C60",
    #                          "/home/iscas/ddz/UIE/data/UIEB/challenging-60-depth",
    #                          image_size=args.image_size)
    ...
    def uie_cond_fn(x_0, t, raw, label):
        ...
        with th.enable_grad():
            ...
            loss = loss - loss_mae + loss_ssim - 0.005 * loss_content + iqa_weight * (0.01*loss_iqa + loss_uranker) #- 0.01 * loss_illuminate #- 0.1 * loss_content #+ 0.1 * loss_style #- 0.05 * loss_illuminate - 0.5 * loss_color #- 0.1 * loss_illuminate + 0.1 * loss_uranker #+ 0.5*loss_style - loss_illuminate - loss_color #+ 0.01 * loss_uranker#- 1.0 * loss_classifier # + 0.001 * loss_iqa
            if t[0].item() % 50 == 0:
    ...
    for i, data in enumerate(UIEDataloader):
        ...
        all_filenames.extend(data[3])
        all_sizes.extend(data[4])
        ...
        #         each_time_sample = ((each_time_sample + 1) * 127.5).clamp(0, 255).to(th.uint8)
        ...
        #     save_process_images(all_res_new, save_dir="data/process/", save_name=data[3])
        # if i <= 2:
        #     all_x0_new = []
        #     for each_time_sample in all_x0:
        #         # each_time_sample = (normalize_img(each_time_sample) * 255).to(th.uint8)
        #         each_time_sample = ((each_time_sample + 1) * 127.5).clamp(0, 255).to(th.uint8)
        #         each_time_sample = each_time_sample.permute(0, 2, 3, 1)
        #         each_time_sample = each_time_sample.contiguous()
        #         all_x0_new.append(each_time_sample.cpu().numpy())
            
        #     save_process_images(all_x0_new, save_dir="data/process1/", save_name=data[3])
        # if i == 2:
        #     break
    ...
    if dist.get_rank() == 0:
        ...
        save_images(arr, all_filenames, save_dir="/home/iscas/ddz/UIE/data/UIEB/All_Results/DDPM_Fusion/C60/") # /home/iscas/ddz/UIE/data/UIEB/All_Results/DDPM_UIEC2Net/C60/
