...
class GaussianDiffusion:
    ...
    def condition_mean_uie(self, cond_fn, p_mean_var, x, pred_xstart, t, model_kwargs=None):
        """
        Compute the mean for the previous step, given a function cond_fn that
        computes the gradient of a conditional log probability with respect to
        x. In particular, cond_fn computes grad(log(p(y|x))), and we want to
    ...
    def condition_score_uie(self, cond_fn, p_mean_var, x, pred_xstart, t, model_kwargs=None):
        """
        Compute what the p_mean_variance output would have been, should the
        model's score function be conditioned by cond_fn.
    ...
    def p_sample(
        ...
        if cond_fn is not None:
            out["mean"] = self.condition_mean_uie(
    ...
    def ddim_sample(
        ...
        if cond_fn is not None:
            out = self.condition_score_uie(cond_fn, out, x, out["pred_xstart"], t, model_kwargs=model_kwargs)
